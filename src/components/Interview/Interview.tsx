...
import { InterviewType, fetchAnswer, fetchInterview } from '../../store/reducers/interview/interviewActionCreator';
      ...
      <div className='interview__content'>
        <div className='interview__qa-container interview__question'>
          <h2 className='interview__subtitle page__text'>
            Вопрос {questionCount + 1} <span className='interview__subtitle-span'>/ {interview?.questions.length}</span>
          </h2>
          <p className='interview__question-text page__text'>
            {interview?.questions[questionCount].text}
          </p>
        </div>
        <div className={`interview__qa-container interview__answer ${answer ? 'interview__answer_type_visible'
          : ''} page__text`}>
          <h2 className='interview__subtitle page__text'>
            Правильный ответ
          </h2>
          <p ref={answerRef} className='interview__answer-text page__text'>
            {answer}
          </p>
        </div>
        <form className='interview__form'>
          ...
          <textarea placeholder='Ввести ответ' className='interview__user-answer page__text' name='interview'>

          </textarea>
          <button
            onClick={answer ? interview?.questions.length === questionCount + 1 ? handleInterviewFinished : handleGoToNextQuestion : handleShowAnswer} type='button'
        ...
        <img src={interviewImage} alt='Квадратики' className='interview__image' />
        <button onClick={answer ? interview?.questions.length === questionCount + 1 ? handleInterviewFinished : handleGoToNextQuestion : handleShowAnswer} type='button' className='interview__button interview__button_type_mobile sections__link'>{answer ? interview?.questions.length === questionCount + 1 ? 'Завершить испытание' : 'Следующий вопрос' : 'Показать правильный ответ'}</button>
      </div>
    </section>
  )
};
